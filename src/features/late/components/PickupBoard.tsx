...
type Item = { id: string; vin: string; assignedDate: string; createdAt: string };
type AddResult = { added: number; duplicates: number; skippedCarrier: number; notLateYet: number };
...
export function PickupBoard() {
    ...
    const load = useCallback(async () => {
        try {
            const res = await fetch("/api/late-pickups");
            if (res.ok) {
    ...
    async function remove(id: string) {
        ...
        await fetch(`/api/late-pickups?id=${id}`, { method: "DELETE" });
    }
    ...
    return (
        <main className="mx-auto max-w-4xl space-y-5 px-6 py-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-lg font-semibold">Late pickups</h1>
                    <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                        Week of {weekStart ? pretty(weekStart) : ""} · counted from 2 days after the assigned date
                    </p>
                </div>
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-3 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <div className="mb-1 text-xs font-medium text-gray-600 dark:text-gray-400">
                    Paste the pickups page from ACP
                </div>
                <textarea
                    onPaste={onPaste}
                    ...
                    placeholder="Paste here — only pickups 2+ days past the assigned date are counted…"
                />
                ...
                {result && (
                    <p className="mt-2 text-xs text-gray-600 dark:text-gray-300">
                        ...
                        {result.notLateYet > 0 && <> · {result.notLateYet} not late yet</>}
                        {result.skippedCarrier > 0 && <> · {result.skippedCarrier} skipped (excluded carrier)</>}
                    </p>
                )}
            </div>
                    ...
                    <ul className="divide-y divide-gray-100 dark:divide-gray-800">
                        {items.map((it) => (
                            <li key={it.id} className="flex items-center justify-between px-4 py-2.5">
                                ...
                                <span className="text-xs text-gray-400">assigned {pretty(it.assignedDate)}</span>
                            </div>
                            <button
